...
public class FileSearcherUtil {
    ...
    private final String TAG = FileSearcherUtil.class.getSimpleName();
    ...
    private final PublishSubject<Integer> searchProgress = PublishSubject.create();
    ...
    public Single<List<Object>> searchAndParseFilesWithExtensionRecursively(Context context, Function<File, Object> parserFunction, String fileExtension, String[] mediaStoreSelectionArgs) {
    ...
    private void searchFileSystemDirect(Context context, AtomicInteger totalFiles, AtomicInteger processedFiles, List<File> fileList) {
        ...
        for (File storageDir : storageDirs) {
            searchAndParseFilesWithExtensionRecursively(storageDir, fileList, totalFiles, processedFiles);
        }
    ...
    private void searchAndParseFilesWithExtensionRecursively(File directory, List<File> fileList, AtomicInteger totalFiles, AtomicInteger processedFiles) {
        if (directory == null || !directory.exists()) {
        ...
        try {
            ...
            if (files != null) {
                for (File file : files) {
                    if (file.isDirectory()) {
                        searchAndParseFilesWithExtensionRecursively(file, fileList, totalFiles, processedFiles);
                    } else {
    ...
    private void updateProgress(AtomicInteger processedFiles, AtomicInteger totalFiles) {
        if (totalFiles.get() > 0) {
            ...
            searchProgress.onNext(progress);
        }
    }

    /**
     * Returns an Observable that emits the search progress as a percentage.
     *
     * @return Observable emitting integers from 0 to 100 representing search progress
     */
    public Observable<Integer> getSearchProgress() {
        return searchProgress.hide();
    }
}
